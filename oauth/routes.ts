...
async function handleCallback(req: Request, res: Response): Promise<void> {
  ...
  // If client provided a redirect URI, do a proper HTTP 302 redirect.
  // MCP clients (Claude Desktop, etc.) listen on a local HTTP server and
  // need a standard redirect — JavaScript redirects in HTML pages don't work.
  if (storedState.clientRedirectUri) {
